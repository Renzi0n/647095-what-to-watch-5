import reviews from './reviews';

export default [
  {
    title: `Мулан`,
    previewSrc: `https://kinohod.ru/o/88/d3/88d3054f-8fd3-4daf-8977-bb4bc8b95206.jpg`,
    genre: `Фэнтези`,
    year: `2020`,
    posterSrc: `https://st.kp.yandex.net/im/poster/3/4/7/kinopoisk.ru-Mulan-3477206.jpg`,
    videoSrc: `https://www.youtube.com/watch?v=rVn2NCgCVLU`,
    overview: {
      rate: 5.9,
      votes: 230,
      text: `История о бесстрашной молодой девушке, которая выдаёт себя за мужчину,
        чтобы вступить в ряды армии,
        противостоящей Северным захватчикам, надвигающимся на Китай.
        Старшая дочь храброго воина Хуа, Мулан — энергичная и решительная девушка.`,
      director: `Ники Каро`,
      starring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли`
    },
    details: {
      runTime: `2h 0m`,
      fullStarring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли, Йосон Ань, Ма Ци, Розалинд Чао, Чжэн Пэйпэй, Сюзанна Тан`
    },
    reviews: Object.assign({}, reviews),
  },
  {
    title: `Bohemian Rhapsody`,
    previewSrc: `../markup/img/bohemian-rhapsody.jpg`,
    genre: `Фэнтези`,
    year: `2019`,
    posterSrc: `https://www.film.ru/sites/default/files/styles/thumb_1024x450/public/trailers_frame/bohemian_rhapsody.jpg `,
    videoSrc: `https://upload.wikimedia.org/wikipedia/commons/transcoded/b/b3/Big_Buck_Bunny_Trailer_400p.ogv/Big_Buck_Bunny_Trailer_400p.ogv.360p.webm`,
    overview: {
      rate: 8.2,
      votes: 220,
      text: `«Bohemian Rhapsody» — песня британской рок-группы Queen из альбома A Night At The Opera; написана Фредди Меркьюри в 1975 году.
      Песня имеет необычную музыкальную форму. Её можно разбить на шесть разных по стилю частей, которые не делятся на куплеты и припевы и
      представляют отдельные музыкальные направления: оперу и балладу, пение а капелла и рок.`,
      director: `Ники Каро`,
      starring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли`
    },
    details: {
      runTime: `2h 22m`,
      fullStarring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли, Йосон Ань, Ма Ци, Розалинд Чао, Чжэн Пэйпэй, Сюзанна Тан`
    },
    reviews: Object.assign({}, reviews),
  },
  {
    title: `Мулан`,
    previewSrc: `https://kinohod.ru/o/88/d3/88d3054f-8fd3-4daf-8977-bb4bc8b95206.jpg`,
    genre: `Фэнтези`,
    year: `2020`,
    posterSrc: `https://st.kp.yandex.net/im/poster/3/4/7/kinopoisk.ru-Mulan-3477206.jpg`,
    videoSrc: `https://download.blender.org/durian/trailer/sintel_trailer-480p.mp4`,
    overview: {
      rate: 5.5,
      votes: 230,
      text: `«Bohemian Rhapsody» — песня британской рок-группы Queen из альбома A Night At The Opera; написана Фредди Меркьюри в 1975 году.
      Песня имеет необычную музыкальную форму. Её можно разбить на шесть разных по стилю частей, которые не делятся на куплеты и припевы и
      представляют отдельные музыкальные направления: оперу и балладу, пение а капелла и рок.`,
      director: `Ники Каро`,
      starring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли`
    },
    details: {
      runTime: `2h 0m`,
      fullStarring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли, Йосон Ань, Ма Ци, Розалинд Чао, Чжэн Пэйпэй, Сюзанна Тан`
    },
    reviews: Object.assign({}, reviews),
  },
  {
    title: `The Grand Budapest Hotel`,
    previewSrc: `../markup/img/the-grand-budapest-hotel-poster.jpg`,
    genre: `Drama`,
    year: `2016`,
    posterSrc: `https://st.kp.yandex.net/im/poster/3/4/7/kinopoisk.ru-Mulan-3477206.jpg`,
    videoSrc: `https://download.blender.org/durian/trailer/sintel_trailer-480p.mp4`,
    overview: {
      rate: 6.4,
      votes: 230,
      text: `In the 1930s, the Grand Budapest Hotel is a popular European ski resort,
      presided over by concierge Gustave H. (Ralph Fiennes). Zero, a junior lobby boy, becomes Gustave's friend and protege.
      Gustave prides himself on providing first-class service to the hotel's guests, including satisfying the sexual
      needs of the many elderly women who stay there. When one of Gustave's lovers dies mysteriously,
      Gustave finds himself the recipient of a priceless painting and the chief suspect in her murder.`,
      director: `Ники Каро`,
      starring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли`
    },
    details: {
      runTime: `34h 0m`,
      fullStarring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли, Йосон Ань, Ма Ци, Розалинд Чао, Чжэн Пэйпэй, Сюзанна Тан`
    },
    reviews: Object.assign({}, reviews),
  },
  {
    title: `Revenant`,
    previewSrc: `../markup/img/revenant.jpg`,
    genre: `Фэнтези`,
    year: `2017`,
    posterSrc: `https://st.kp.yandex.net/im/poster/3/4/7/kinopoisk.ru-Mulan-3477206.jpg`,
    videoSrc: `https://www.youtube.com/watch?v=rVn2NCgCVLU`,
    overview: {
      rate: 7.7,
      votes: 230,
      text: `История о бесстрашной молодой девушке, которая выдаёт себя за мужчину,
        чтобы вступить в ряды армии,
        противостоящей Северным захватчикам, надвигающимся на Китай.
        Старшая дочь храброго воина Хуа, Мулан — энергичная и решительная девушка.`,
      director: `Ники Каро`,
      starring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли`
    },
    details: {
      runTime: `2h 0m`,
      fullStarring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли, Йосон Ань, Ма Ци, Розалинд Чао, Чжэн Пэйпэй, Сюзанна Тан`
    },
    reviews: Object.assign({}, reviews),
  },
  {
    title: `Aviator`,
    previewSrc: `../markup/img/aviator.jpg`,
    genre: `Боевик`,
    year: `2020`,
    posterSrc: `https://st.kp.yandex.net/im/poster/3/4/7/kinopoisk.ru-Mulan-3477206.jpg`,
    videoSrc: `https://www.youtube.com/watch?v=rVn2NCgCVLU`,
    overview: {
      rate: 9.2,
      votes: 230,
      text: `«Bohemian Rhapsody» — песня британской рок-группы Queen из альбома A Night At The Opera; написана Фредди Меркьюри в 1975 году.
      Песня имеет необычную музыкальную форму. Её можно разбить на шесть разных по стилю частей, которые не делятся на куплеты и припевы и
      представляют отдельные музыкальные направления: оперу и балладу, пение а капелла и рок.`,
      director: `Ники Каро`,
      starring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли`
    },
    details: {
      runTime: `2h 0m`,
      fullStarring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли, Йосон Ань, Ма Ци, Розалинд Чао, Чжэн Пэйпэй, Сюзанна Тан`
    },
    reviews: Object.assign({}, reviews),
  },
  {
    title: `Mindhunter`,
    previewSrc: `../markup/img/mindhunter.jpg`,
    genre: `Фэнтези`,
    year: `2020`,
    posterSrc: `https://st.kp.yandex.net/im/poster/3/4/7/kinopoisk.ru-Mulan-3477206.jpg`,
    videoSrc: `https://www.youtube.com/watch?v=rVn2NCgCVLU`,
    overview: {
      rate: 8.3,
      votes: 230,
      text: `История о бесстрашной молодой девушке, которая выдаёт себя за мужчину,
        чтобы вступить в ряды армии,
        противостоящей Северным захватчикам, надвигающимся на Китай.
        Старшая дочь храброго воина Хуа, Мулан — энергичная и решительная девушка.`,
      director: `Ники Каро`,
      starring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли`
    },
    details: {
      runTime: `2h 0m`,
      fullStarring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли, Йосон Ань, Ма Ци, Розалинд Чао, Чжэн Пэйпэй, Сюзанна Тан`
    },
    reviews: Object.assign({}, reviews),
  },
  {
    title: `Midnight Special`,
    previewSrc: `../markup/img/midnight-special.jpg`,
    genre: `Фэнтези`,
    year: `2020`,
    posterSrc: `https://st.kp.yandex.net/im/poster/3/4/7/kinopoisk.ru-Mulan-3477206.jpg`,
    videoSrc: `https://www.youtube.com/watch?v=rVn2NCgCVLU`,
    overview: {
      rate: 4.9,
      votes: 230,
      text: `In the 1930s, the Grand Budapest Hotel is a popular European ski resort,
      presided over by concierge Gustave H. (Ralph Fiennes). Zero, a junior lobby boy, becomes Gustave's friend and protege.
      Gustave prides himself on providing first-class service to the hotel's guests, including satisfying the sexual
      needs of the many elderly women who stay there. When one of Gustave's lovers dies mysteriously,
      Gustave finds himself the recipient of a priceless painting and the chief suspect in her murder.`,
      director: `Ники Каро`,
      starring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли`
    },
    details: {
      runTime: `2h 0m`,
      fullStarring: `Лю Ифэй, Донни Йен, Гун Ли, Джет Ли, Джейсон Скотт Ли, Йосон Ань, Ма Ци, Розалинд Чао, Чжэн Пэйпэй, Сюзанна Тан`
    },
    reviews: Object.assign({}, reviews),
  },
];
